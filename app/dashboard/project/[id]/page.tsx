...
import { CodeExplorer } from "@/components/editor/code-explorer"
...
import { detectGenerationMode } from "@/lib/ai/code-parser"
...
export default function EditorPage() {
  ...
  const [viewerFile, setViewerFile] = useState<GeneratedFile | null>(null)
  const [activeCodeExplorerFile, setActiveCodeExplorerFile] = useState<string | null>(null)
  ...
  const handleSmartGenerate = useCallback(async (prompt: string, model: string) => {
    setIsGenerating(true)
    setSmartGenError(null)

    try {
      // Detect mode based on current files
      const mode = detectGenerationMode(generatedFiles)
      setGenerationMode(mode)

      // Call smart generate API
      ...
      // Save to project
      await saveFiles(data.files || [], prompt)
    } catch (error) {
      const message = error instanceof Error ? error.message : "Terjadi kesalahan saat membuat kode"
      setSmartGenError(message)
      pushErrorLog("generate", message)
      console.error("[v0] Smart generate error:", error)
    } finally {
      setIsGenerating(false)
    }
  }, [projectId, generatedFiles, saveFiles, pushErrorLog])

  const handleSmartGenerateFileSelect = useCallback((filePath: string) => {
    setActiveCodeExplorerFile(filePath)
  ...
  const handleSendMessage = useCallback(async (
    ...
    const previewContext = buildPreviewContextPacket({
      ...
      activeTab: activePreviewTab === "explorer" ? "code" : activePreviewTab,
    ...
    try {
      ...
      if (Array.isArray(previewFilesPayload.previewFiles)) {
  ...
  }, [buildProviderStatusFromError, createIdempotencyKey, latestPreviewError, messages, projectId, pushErrorLog])
  ...
  const chatDefaultSize = layoutPreset === "prompt" ? 34 : layoutPreset === "preview" ? 30 : 32
  const previewDefaultSize = layoutPreset === "prompt" ? 66 : layoutPreset === "preview" ? 70 : 68
