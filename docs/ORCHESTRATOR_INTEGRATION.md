...
1. **Pre-generation Check**: System verifies user has at least 5,000 IDR balance
2. **Post-generation Charge**: Upon successful generation, 5,000 IDR is deducted from user balance
...
export class OrchestratorProvider {
  ...
  static getModel(): string // Returns "deepseek/deepseek-v4-flash"
...
```typescript
const balanceCheck = await GenerateBillingService.checkBalance(userId, 5000)
...
|---------|-------------|-----|
| Model | deepseek-v4-flash | v0-web-generator |
| Cost | 5,000 IDR | 5,000 IDR |
| Speed | Fast | Very Fast |
| Specialization | General code generation | Web UI/Frontend |
| Provider | Deepseek-powered | V0 platform |
...
2. Switch model in UI from "V0" to "Orchestrator - Deepseek V4 Flash"
