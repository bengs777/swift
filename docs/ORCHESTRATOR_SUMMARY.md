...
A fully integrated AI code generation provider using **Orchestrator** with the **deepseek/deepseek-v4-flash** model, priced at **5,000 IDR per request**.
...
- `lib/ai/providers/orchestrator-provider.ts` - Provider service wrapper
  ...
  - Validates responses
  - Fixed cost: 5,000 IDR per request
...
### Billing
- `lib/services/generate-billing.service.ts` - Added Orchestrator cost breakdown (5,000 IDR)

### Models API
- `app/api/models/route.ts` - Added Orchestrator model to available models list
  - Model appears as "Orchestrator - Deepseek V4 Flash (5000/req)"
...
```
User enters prompt → Selects "Orchestrator - Deepseek V4 Flash" → Clicks Generate
```

### 2. System Validates
```
Check Authentication → Check Balance (need 5,000 IDR) → Check Provider Config
```
...
### 4. Billing
```
Charge 5,000 IDR → Log transaction → Update user balance → Return files and new balance
```
...
✅ **Fixed Pricing** - 5,000 IDR per request (no variable costs)
...
- Shows "Orchestrator - Deepseek V4 Flash (5000/req)"
...
| Event | Cost | Notes |
|-------|------|-------|
| Successful Generation | 5,000 IDR | Always charged |
...
|----------|-------|------|-------|
| Orchestrator | deepseek-v4-flash | 5,000 IDR | Fast ⚡ |
| V0 | v0-web-generator | 5,000 IDR | Very Fast ⚡⚡ |
...
- Shows cost information (5000/req)
...
### Check Balance
```typescript
const balance = await GenerateBillingService.checkBalance(userId, 5000)
...
3. **Test in UI**
   - Open editor
   - Select "Orchestrator - Deepseek V4 Flash" from model dropdown
...
Orchestrator Provider Service
    ↓
Orchestrator API (deepseek-v4-flash)
    ↓
Return Generated Files
    ↓
Charge User (5,000 IDR)
    ↓
Log Transaction
    ↓
