...
const handleGenerate = async (prompt: string, model: string) => {
  if (model === "v0-web-generator") {
    ...
    if (data.success) {
      ...
      // Show: "Generated! Cost: 5000 IDR, New balance: X"
    } else {
...
| Item | Value |
|------|-------|
| Cost per generation | 5,000 IDR |
| Currency | IDR (Indonesian Rupiah) |
| Billing model | Pay-per-use |
| Minimum balance | 5,000 IDR |
...
# Response:
# { "userId": "123", "balance": 100000, "costPerGeneration": 5000 }
```
...
# Should include:
# { "key": "v0-web-generator", "provider": "v0", "label": "V0 Web Generator (5000/req)" }
```
...
### "Insufficient balance" (402)
- Expected behavior
- Show user: "Balance tidak cukup. Diperlukan Rp 5.000"
- Link to top-up page
...
5. **Verify**:
   - Files generated
   - Balance = 95,000 IDR (100,000 - 5,000)
