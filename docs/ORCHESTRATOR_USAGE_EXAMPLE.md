  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 402) {
        ...
        throw new Error(`Insufficient balance. Need 5000 IDR, have ${error.currentBalance}`)
  ...
  return (
    <div className="space-y-4">
      ...
      <Button
        ...
        className="w-full"
      >
        {loading ? "Generating..." : "Generate (5000 IDR)"}
      </Button>
    </div>
  )
}
...
async function getUserOrchestratorStats(userId: string) {
  ...
  return {
    ...
    estimatedGenerationsLeft: Math.floor((user?.balance || 0) / 5000),
